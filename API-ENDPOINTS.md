...
| `PUT` | `/api/grow-cycles/:id` | GrowFormView edit (name, controllerId, isActive) |
...
### Grow Cycle Creation
`POST /api/grow-cycles` accepts only `{ name, controllerId, isActive? }` and **auto-generates** 4 default phases:
